...
use crate::camera::{Camera, Mat4, DEFAULT_FOV};
...
impl Renderer {
    pub async fn new(window: Arc<Window>) -> Result<Self, RenderError> {
        ...
        log::info!("Wisp GPU: {} ({:?})", adapter.get_info().name, adapter.get_info().backend);
        ...
        let bl = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
        ...
        Ok(Self {
            surface, device, queue, config, size, pipeline, depth,
            cube_v, cube_i, food_v, food_i, ub, bg,
        })
    }

    fn make_depth(d: &wgpu::Device, w: u32, h: u32) -> wgpu::TextureView {
    ...
    pub fn render(&mut self, fly: &Fly, food: &Food) -> Result<(), wgpu::SurfaceError> {
        let f = fly.forward();
        let cam = Camera {
            position: [fly.position[0] - f[0] * 5.0, 3.0, fly.position[2] - f[2] * 5.0],
            target: [fly.position[0] + f[0] * 1.5, 0.25, fly.position[2] + f[2] * 1.5],
            aspect: self.size.width as f32 / self.size.height.max(1) as f32,
            fov_y: DEFAULT_FOV,
            near: 0.1,
            far: 40.0,
        };
        ...
        self.write(5, vp, Mat4::translation(fly.position[0], fly.position[1], fly.position[2]).mul(Mat4::rotation_y(fly.rotation_y)).mul(Mat4::scale(0.8, 0.28, 1.3)), [0.60, 0.20, 0.08, 1.0]);
        self.write(6, vp, Mat4::translation(food.position[0], food.position[1], food.position[2]).mul(Mat4::scale(0.6, 0.6, 0.6)), [0.95, 0.72, 0.10, 1.0]);
        ...
        let mut enc = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("frame") });
        {
            ...
            pass.set_bind_group(0, &self.bg, &[6 * STRIDE as u32]);
