...
const CUBE_V: &[Vertex] = &[
    Vertex { p: [-0.5, -0.5, -0.5] }, Vertex { p: [0.5, -0.5, -0.5] },
    Vertex { p: [0.5, 0.5, -0.5] }, Vertex { p: [-0.5, 0.5, -0.5] },
    Vertex { p: [-0.5, -0.5, 0.5] }, Vertex { p: [0.5, -0.5, 0.5] },
    Vertex { p: [0.5, 0.5, 0.5] }, Vertex { p: [-0.5, 0.5, 0.5] },
];
const CUBE_I: &[u16] = &[
    0, 1, 2, 2, 3, 0, 4, 6, 5, 6, 7, 4, 0, 4, 7, 7, 3, 0,
    1, 5, 6, 6, 2, 1, 3, 2, 6, 6, 7, 3, 0, 1, 5, 5, 4, 0,
];

const FOOD_V: &[Vertex] = &[
    Vertex { p: [0.0, 0.7, 0.0] }, Vertex { p: [0.7, 0.0, 0.0] },
    Vertex { p: [0.0, 0.0, 0.7] }, Vertex { p: [-0.7, 0.0, 0.0] },
    Vertex { p: [0.0, 0.0, -0.7] }, Vertex { p: [0.0, -0.7, 0.0] },
];
const FOOD_I: &[u16] = &[
...
#[derive(Debug)]
pub enum RenderError { AdapterUnavailable, Device(String), Surface(String) }
...
impl Renderer {
    pub async fn new(window: Arc<Window>) -> Result<Self, RenderError> {
        ...
        }).await.ok_or(RenderError::AdapterUnavailable)?;
        ...
        }, None).await.map_err(|e| RenderError::Device(e.to_string()))?;

        let caps = surface.get_capabilities(&adapter);
        let format = caps.formats.iter().copied().find(wgpu::TextureFormat::is_srgb).unwrap_or(caps.formats[0]);
        let mode = if caps.present_modes.contains(&wgpu::PresentMode::Fifo) { wgpu::PresentMode::Fifo } else { caps.present_modes[0] };
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("wisp-shader"), source: wgpu::ShaderSource::Wgsl(SHADER.into()),
        });
        let ub = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("object-uniforms"), size: STRIDE * OBJECT_SLOTS,
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST, mapped_at_creation: false,
        });
        let bind_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("object-layout"), entries: &[wgpu::BindGroupLayoutEntry {
                binding: 0, visibility: wgpu::ShaderStages::VERTEX,
                ty: wgpu::BindingType::Buffer { ty: wgpu::BufferBindingType::Uniform, has_dynamic_offset: true, min_binding_size: None }, count: None,
            }],
        });
        let bg = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("object-bind"), layout: &bind_layout,
            entries: &[wgpu::BindGroupEntry { binding: 0, resource: ub.as_entire_binding() }],
        });
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("wisp-pipeline-layout"), bind_group_layouts: &[&bind_layout], push_constant_ranges: &[],
        });
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("wisp-pipeline"), layout: Some(&pipeline_layout),
        ...
        let cube_v = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some("cube-v"), contents: bytemuck::cast_slice(CUBE_V), usage: wgpu::BufferUsages::VERTEX });
        let cube_i = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some("cube-i"), contents: bytemuck::cast_slice(CUBE_I), usage: wgpu::BufferUsages::INDEX });
        let food_v = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some("food-v"), contents: bytemuck::cast_slice(FOOD_V), usage: wgpu::BufferUsages::VERTEX });
        let food_i = device.create_buffer_init(&wgpu::util::BufferInitDescriptor { label: Some("food-i"), contents: bytemuck::cast_slice(FOOD_I), usage: wgpu::BufferUsages::INDEX });

        Ok(Self { surface, device, queue, config, size, pipeline, depth, cube_v, cube_i, food_v, food_i, ub, bg, camera: CameraRig::default() })
    }

    fn make_depth(device: &wgpu::Device, width: u32, height: u32) -> wgpu::TextureView {
        device.create_texture(&wgpu::TextureDescriptor {
            label: Some("depth"), size: wgpu::Extent3d { width: width.max(1), height: height.max(1), depth_or_array_layers: 1 },
    ...
    pub fn render(&mut self, fly: &Fly, food: &Food, dt: f32) -> Result<(), wgpu::SurfaceError> {
        ...
        self.write(3, vp, Mat4::translation(-ARENA_HALF_SIZE, 0.75, 0.0).mul(Mat4::scale(0.35, 1.5, 16.0)), [0.15, 0.18, 0.17, 1.0]);
        self.write(4, vp, Mat4::translation(ARENA_HALF_SIZE, 0.75, 0.0).mul(Mat4::scale(0.35, 1.5, 16.0)), [0.15, 0.18, 0.17, 1.0]);

        let fly_model = Mat4::translation(fly.position[0], fly.position[1], fly.position[2]).mul(Mat4::rotation_y(fly.rotation_y));
        self.write(5, vp, fly_model.mul(Mat4::scale(0.38, 0.30, 0.75)), [0.18, 0.09, 0.04, 1.0]);
        self.write(6, vp, fly_model.mul(Mat4::translation(0.0, 0.0, 0.48)).mul(Mat4::scale(0.30, 0.24, 0.48)), [0.10, 0.05, 0.025, 1.0]);
        self.write(7, vp, fly_model.mul(Mat4::translation(0.0, 0.02, -0.48)).mul(Mat4::scale(0.34, 0.27, 0.52)), [0.24, 0.11, 0.04, 1.0]);
        self.write(8, vp, fly_model.mul(Mat4::translation(0.0, 0.0, 0.88)).mul(Mat4::scale(0.25, 0.23, 0.25)), [0.04, 0.025, 0.015, 1.0]);
        self.write(9, vp, fly_model.mul(Mat4::translation(-0.43, 0.16, 0.02)).mul(Mat4::rotation_z(-0.12)).mul(Mat4::scale(0.75, 0.035, 0.48)), [0.42, 0.45, 0.38, 0.65]);
        self.write(10, vp, fly_model.mul(Mat4::translation(0.43, 0.16, 0.02)).mul(Mat4::rotation_z(0.12)).mul(Mat4::scale(0.75, 0.035, 0.48)), [0.42, 0.45, 0.38, 0.65]);
        self.write(11, vp, Mat4::translation(food.position[0], food.position[1], food.position[2]).mul(Mat4::scale(0.6, 0.6, 0.6)), [0.95, 0.72, 0.10, 1.0]);
