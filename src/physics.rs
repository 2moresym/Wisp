...
pub struct Fly { pub position: [f32;3], pub velocity: [f32;3], pub rotation_y: f32 }

#[derive(Clone, Copy, Debug)]
pub struct Food { pub position: [f32;3], pub radius: f32 }

impl Default for Fly { fn default()->Self { Self{position:[0.0,0.15,0.0],velocity:[0.0;3],rotation_y:0.0} } }
impl Default for Food { fn default()->Self { Self{position:[5.0,0.35,2.0],radius:0.45} } }
...
impl Fly {
    ...
    pub fn visual_input(&self, food:&Food)->VisualInput {
        ...
        VisualInput{angle,distance,lateral}
    }
...
mod tests {
    use super::*;
    #[test] fn collision_radius_works(){ let fly=Fly::default(); let mut food=Food::default(); food.position=[0.5,0.35,0.0]; assert!(fly.collides_with(&food)); food.position=[3.0,0.35,0.0]; assert!(!fly.collides_with(&food)); }
    #[test] fn visual_input_reports_food_ahead(){ let fly=Fly::default(); let mut food=Food::default(); food.position=[4.0,0.35,0.0]; let v=fly.visual_input(&food); assert!(v.angle.abs()<0.01); assert!(v.distance>3.9); }
    #[test] fn motor_moves_forward(){ let mut fly=Fly::default(); fly.apply_motor(0.0,1.0,0.1); fly.integrate(0.1); assert!(fly.position[0]>0.0); }
}
