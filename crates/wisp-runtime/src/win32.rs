...
pub fn abi_address(symbol: &str) -> Option<u64> {
    macro_rules! addr { ($name:literal, $fn_name:ident) => { if symbol.eq_ignore_ascii_case($name) { return Some($fn_name as *const () as usize as u64); } }; }
