...
use wayland_client::{
    delegate_noop,
    protocol::{wl_buffer, wl_keyboard, wl_pointer, wl_registry, wl_seat, wl_shm, wl_surface},
    Connection, Dispatch, EventQueue, QueueHandle, WEnum,
};
use wayland_client::protocol::{wl_compositor, wl_shm_pool};
...
impl State {
    ...
    fn initialize(&mut self, qh: &QueueHandle<Self>, config: &WindowConfig) -> Result<(), WindowError> {
        ...
        if self.shm.is_none() {
            return Err(WindowError::Wayland("wl_shm missing".into()));
        }
    ...
    fn present(&mut self, qh: &QueueHandle<Self>) -> Result<(), WindowError> {
...
impl WaylandWindow {
    pub fn new(config: &WindowConfig) -> Result<Self, WindowError> {
        let connection = Connection::connect_to_env().map_err(|e| WindowError::Wayland(e.to_string()))?;
        let mut event_queue = connection.new_event_queue();
        let qh = event_queue.handle();
        let mut state = State::new(config.width, config.height);

        connection.display().get_registry(&qh, ());
        event_queue.roundtrip(&mut state).map_err(|e| WindowError::Wayland(e.to_string()))?;
    ...
    pub fn show(&mut self) -> Result<(), WindowError> {
        self.event_queue.roundtrip(&mut self.state).map_err(|e| WindowError::Wayland(e.to_string()))?;
        if !self.state.configured {
            return Err(WindowError::Wayland("compositor did not configure surface".into()));
        }
        let qh = self.event_queue.handle();
        self.state.present(&qh)?;
        self.connection.flush().map_err(|e| WindowError::Wayland(e.to_string()))?;
        Ok(())
    }
...
fn create_buffer(
    ...
    let stride = width.checked_mul(4).ok_or_else(|| WindowError::Wayland("shared-memory stride overflow".into()))?;
    let size = stride.checked_mul(height).ok_or_else(|| WindowError::Wayland("shared-memory buffer size overflow".into()))?;
    let counter = SHM_FILE_COUNTER.fetch_add(1, Ordering::Relaxed);
    let path = PathBuf::from(format!("/dev/shm/wisp-window-{}-{}", std::process::id(), counter));
    ...
    file.set_len(size as u64).map_err(|e| WindowError::Wayland(format!("size shm file: {e}")))?;
    let mut pixels = vec![0u8; size as usize];
    for px in pixels.chunks_exact_mut(4) {
        px.copy_from_slice(&[0x30, 0x30, 0x30, 0xff]);
    }
    file.write_all(&pixels).map_err(|e| WindowError::Wayland(format!("write shm buffer: {e}")))?;
    file.flush().map_err(|e| WindowError::Wayland(format!("flush shm buffer: {e}")))?;
    ...
    fn event(state: &mut Self, registry: &wl_registry::WlRegistry, event: wl_registry::Event, _: &(), _: &Connection, qh: &QueueHandle<Self>) {
        if let wl_registry::Event::Global { name, interface, version } = event {
            match interface.as_str() {
                ...
                "wl_seat" => { let _: wl_seat::WlSeat = registry.bind(name, version.min(7), qh, ()); }
                _ => {}
            }
        }
    }
...
impl Dispatch<wl_buffer::WlBuffer, ()> for State {
    fn event(state: &mut Self, buffer: &wl_buffer::WlBuffer, event: wl_buffer::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        if let wl_buffer::Event::Release = event {
            if let Some(index) = state.retired_buffers.iter().position(|old| old == buffer) {
                state.retired_buffers.swap_remove(index);
                buffer.destroy();
            }
        }
    }
    ...
    fn event(_: &mut Self, seat: &wl_seat::WlSeat, event: wl_seat::Event, _: &(), _: &Connection, qh: &QueueHandle<Self>) {
        if let wl_seat::Event::Capabilities { capabilities } = event {
            if let WEnum::Value(caps) = capabilities {
                if caps.contains(wl_seat::Capability::Keyboard) { seat.get_keyboard(qh, ()); }
                if caps.contains(wl_seat::Capability::Pointer) { seat.get_pointer(qh, ()); }
            }
        }
    }
}
        ...
        match event {
            ...
            wl_keyboard::Event::Key { key, state: key_state, .. } => {
                if let WEnum::Value(key_state) = key_state {
                    state.events.push(WindowEvent::KeyboardInput { key, pressed: key_state == wl_keyboard::KeyState::Pressed });
                }
            }
            _ => {}
        }
...
impl Dispatch<wl_pointer::WlPointer, ()> for State {
    fn event(state: &mut Self, _: &wl_pointer::WlPointer, event: wl_pointer::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
        match event {
            wl_pointer::Event::Motion { surface_x, surface_y, .. } => state.events.push(WindowEvent::MouseMoved { x: surface_x, y: surface_y }),
            wl_pointer::Event::Button { button, state: button_state, .. } => {
                if let WEnum::Value(button_state) = button_state {
                    state.events.push(WindowEvent::MouseButton { button, pressed: button_state == wl_pointer::ButtonState::Pressed });
                }
            }
            _ => {}
        }
